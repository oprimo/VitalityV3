...
-----------------------------------------------------------------------------------------------------------------------------------------
src = {}
Tunnel.bindInterface("vitality_dealership",src)
vCLIENT = Tunnel.getInterface("vitality_dealership")
	...
	if user_id then
		...
		if vRP.hasPermission(user_id,"conce.permissao") then
			if parseInt(maxvehs[1].qtd) >= parseInt(maxgars[1].garagem) + 100 then
				TriggerClientEvent("Notify",source,"importante","Importante","Atingiu o número máximo de veículos em sua garagem.",8000)
				return
			end
		...
		elseif vRP.hasPermission(user_id,"infinity.permissao") then
			if parseInt(maxvehs[1].qtd) >= parseInt(maxgars[1].garagem) + 6 then
				TriggerClientEvent("Notify",source,"importante","Importante","Atingiu o número máximo de veículos em sua garagem.",8000)
				return
			end
		elseif vRP.hasPermission(user_id,"infinityp.permissao") then
		...
		if vehicle[1] then
			TriggerClientEvent("Notify",source,"importante","Importante","Você já possui um <b>"..vRP.vehicleName(name).."</b> em sua garagem.",10000)
			return
		else
			local rows2 = vRP.query("creative/get_estoque",{ vehicle = name })
			if parseInt(rows2[1].quantidade) <= 0 then
				TriggerClientEvent("Notify",source,"importante","Importante","Estoque de <b>"..vRP.vehicleName(name).."</b> indisponivel.",8000)
				return
			end
			if vRP.tryFullPayment(user_id,vRP.vehiclePrice(name)*0.95) then
				...
				TriggerClientEvent("Notify",source,"importante","Importante","Você comprou um <b>"..vRP.vehicleName(name).."</b> por <b>$ "..vRP.format(parseInt(vRP.vehiclePrice(name)*0.95)).." dólares</b>.",10000)
				...
				TriggerClientEvent("Notify",source,"aviso","Aviso","Dinheiro insuficiente.",10000)
			end
		end
	end
end
...
function src.sellDealer(name)
	...
	if user_id then
		...
		if vehicle[1] then
			...
			TriggerClientEvent("Notify",source,"sucesso","Sucesso","Você vendeu um <b>"..vRP.vehicleName(name).."</b> por <b>$"..vRP.format(parseInt(vRP.vehiclePrice(name)*0.50)).." dólares</b>.",10000)
			src.updateVehicles(name,vRP.vehicleType(name))
