$(document).ready(function () {
	...
	document.onkeyup = function (data) {
		if (data.which == 27) {
			$.post("http://vitality_dealership/dealerClose", JSON.stringify({}), function (datab) { });
		}
	};
});
...
const updateCarros = () => {
	$.post("http://vitality_dealership/requestCarros", JSON.stringify({}), (data) => {
		let i = 0;
...
const updateMotos = () => {
	$.post("http://vitality_dealership/requestMotos", JSON.stringify({}), (data) => {
		let i = 0;
		const nameList = data.veiculos.sort((a, b) => (a.nome > b.nome) ? 1 : -1);
		$('#inicio').html(`
			<div class="comprar">COMPRAR</div>
...
const updateImport = () => {
	$.post("http://vitality_dealership/requestImport", JSON.stringify({}), (data) => {
		let i = 0;
		const nameList = data.veiculos.sort((a, b) => (a.nome > b.nome) ? 1 : -1);
		$('#inicio').html(`
			<div class="comprar">COMPRAR</div>
...
const updateExclusive = () => {
	$.post("http://vitality_dealership/requestExclusive", JSON.stringify({}), (data) => {
		let i = 0;
		const nameList = data.veiculos.sort((a, b) => (a.nome > b.nome) ? 1 : -1);
		$('#inicio').html(`
			<div class="comprar">COMPRAR</div>
			<div class="obs">Para efetuar uma <b>compra</b> selecione um modelo abaixo e clique em <b>comprar</b>, o sistema vai efetuar as checagens necessárias e se você possuir o valor do veículo ele compra automaticamente.</div>
			<div class="title">EXCLUSIVO</div>
			${nameList.map((item) => (`
				<div class="model" data-name-key="${item.k}">
					<div class="id">${i = i + 1}</div>
					<div class="name">${item.nome}</div>
...
const updatePossuidos = () => {
	$.post("http://vitality_dealership/requestPossuidos", JSON.stringify({}), (data) => {
		let i = 0;
		const nameList = data.veiculos.sort((a, b) => (a.nome > b.nome) ? 1 : -1);
		$('#inicio').html(`
...
$(document).on("click", ".comprar", function () {
	let $el = $('.model.active');
	if ($el) {
		$.post("http://vitality_dealership/buyDealer", JSON.stringify({
...
$(document).on("click", ".vender", function () {
	let $el = $('.model.active');
	if ($el) {
		$.post("http://vitality_dealership/sellDealer", JSON.stringify({
			name: $el.attr('data-name-key')
		}));
	}
});