...
$(document).ready(function() {
  ...
  document.onkeyup = function(data) {
    if (data.which == 27) {
      $.post("http://vitality_inventory/invClose", JSON.stringify({}), function(datab) {});
    }
  };
});
...
var updateMochila = () => {
  ...
  $.post("http://vitality_inventory/requestMochila", JSON.stringify({}), data => {
...
$(document).ready(function() {
  ...
  $(document).on('click', '.options .option', function() {
    ...
    if(event == 'send' || event == 'drop') {
      ...
      var amount = 1;
      $.post(
        "http://vitality_inventory/useItem",
        JSON.stringify({
  ...
  $(document).on('click', '.amount-option button', function() {
    ...
    if(event == 'send') {
      ...
      $.post(
        "http://vitality_inventory/" + paramUrl,
        JSON.stringify({
          item: $el.data("item-key"),
          amount
        })
