...
local webhookbancocel = "https://discord.com/api/webhooks/784790082988933140/2xg-BUDcn5IVUMIBdsDu6zFEps_A5iV-WL0875ExOUzAQiK76GqQpiJofs4Bls8KpJmf"
...
function src.checkItemPhone()
    ...
    if user_id then
        if vRP.getInventoryItemAmount(user_id,"celular") >= 1 then
            ...
            TriggerClientEvent("Notify",source,"negado","Negado","Você não possui um celular em sua mochila.") 
            return false
        end
    end
end
	...
	if user_id then
		...
		if phone == "911" then
			if inEmergency[user_id] == "911" then 
				TriggerClientEvent("Notify",source,"negado","Negado","Já Existe um chamado sendo averiguado!")
				return
			end
		...
		elseif phone == "ADM" then
			...
			players = vRP.getUsersByPermission("adminstrador.permissao")	
...
AddEventHandler('bank:transfer', function(id, amount)
	...
	if amount <= 0 then
		return TriggerClientEvent("Notify",_source,"sucesso","Sucesso","Você digitou uma quantia inválida.")
	end
    local identity = vRP.getUserIdentity(user_id)
    local identityT = vRP.getUserIdentity(tonumber(id))    
    if targetPlayer == nil then
        return TriggerClientEvent("Notify",_source,"sucesso","Sucesso","Usuario invalido.")
    else
		...
		if tonumber(user_id) ~= tonumber(id) then
			if myBank >= amount then
				...
				TriggerClientEvent("Notify",targetPlayer,"financeiro","financeiro","Você recebeu $" .. amount .." de "  ..identity.name.. " ID: " .. tostring(user_id))
				TriggerClientEvent("Notify",_source,"financeiro","Financeiro","Tranferencia sucedida")       
				SendWebhookMessage(webhookbancocel,"```prolog\n[ID]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[ENVIOU CELULAR]: $"..vRP.format(parseInt(amount)).."\n[PARA]:"..tonumber(user_id).."  "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
			else
				TriggerClientEvent("Notify",_source,"financeiro","Financeiro","SEM MONEY") 
			end
		end
    end
end)