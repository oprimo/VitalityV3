...
-----------------------------------------------------------------------------------------------------------------------------------------
func = {}
Tunnel.bindInterface("vitality_junkyard",func)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookroubolixao = "https://discord.com/api/webhooks/793520052166066207/LLIAgXrEHMRSVQLWW7IEdMWRwNBDTNappdvqfnL2jSMRsuFyhbNyO_W5lvB9xJfG8HSh"
	...
	if user_id then
		...
		if #policia >= 10 and not vRPclient.changingClothes(source) then
			if timers == 0 or not timers then
				timers = 900
				TriggerClientEvent('iniciandojunkyard',source,id,x,y,z)
					...
					for l,w in pairs(policia) do
						...
						if player then
							async(function()
								...
								TriggerClientEvent('chatMessage',player,"190",{64,64,255},"O roubo começou no ^1Ferro Velho de LS^0, dirija-se até o local e intercepte o assaltante.")
