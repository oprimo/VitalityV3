...
-----------------------------------------------------------------------------------------------------------------------------------------
local locais = {
	{ ['id'] = 1, ['x'] = 1982.36, ['y'] = 3053.44, ['z'] = 47.22 },
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- ROTEIRO DO ROUBO
-----------------------------------------------------------------------------------------------------------------------------------------
			...
			for k,v in pairs(locais) do
				if Vdist(v.x,v.y,v.z,x,y,z) <= 1 and not andamento then
					...
					if IsControlJustPressed(0,38) and func.checkPermission() then
						func.checkRobberyYellow(v.id,v.x,v.y,v.z)
					end
				end
			end
		end
...
-- INICIANDO O ROUBO
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("iniciandoyellow")
AddEventHandler("iniciandoyellow",function(id,x,y,z)
	andamento = true
	segundos = 10
	id_loja = id
end)
