...
-- CHECKROBBERY
-----------------------------------------------------------------------------------------------------------------------------------------
function func.checkRobberyYellow(id,x,y,z)
	local source = source
...
function func.giveAwardYellow()
	...
	if user_id then
		...
		if randlist >= 90 then
			local randitem = math.random(#armalista)
			TriggerClientEvent("Notify",source,"sucesso","Sucesso","Você recebeu <b>"..armalista[randitem].qtd.."</b>x <b>"..armalista[randitem].name.."</b>.",8000)
			SendWebhookMessage(webhookyellow,"```prolog\n[ID]: "..user_id.." "..identity.name.." "..identity.firstname.."\n [ROUBOU]: "..armalista[randitem].qtd.." "..armalista[randitem].name.."  "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
			vRP.giveInventoryItem(user_id,armalista[randitem].index,armalista[randitem].qtd)
		else
