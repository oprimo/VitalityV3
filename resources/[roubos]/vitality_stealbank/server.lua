...
-----------------------------------------------------------------------------------------------------------------------------------------
func = {}
Tunnel.bindInterface("vitality_stealbank",func)
-----------------------------------------------------------------------------------------------------------------------------------------
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookbankrobbery = "https://discord.com/api/webhooks/784774552412618772/MOBLq7ermCSOLnZL7kuow67eUftZC1wk9yob3f-qlSV6mWy01aE9O_g0S6_0NHmjMT_o"
...
function func.CheckPayment(bankId)
    ...
    if user_id then
        if parseInt(moneyHeist[bankId]) <= parseInt(BankHeists[bankId]["Money"]) then
            local cashRecieved = math.random(450000)
            moneyHeist[bankId] = parseInt(moneyHeist[bankId]) + cashRecieved
            vRP.giveInventoryItem(user_id,"dinheirosujo",cashRecieved)
            TriggerClientEvent("Notify",source,"sucesso","Sucesso","Recebeu <b>$"..vRP.format(parseInt(cashRecieved)).." dólares sujos</b>.")
        else
...
function func.CheckPolice()
    ...
    if user_id then
        ...
        elseif (os.time()-timedown) <= 7200 then
            TriggerClientEvent("Notify",source,"aviso","Aviso","Os cofres estão vazios, aguarde <b>"..vRP.format(parseInt((0-(os.time()-timedown)))).." segundos</b> até que os civis efetuem depositos.",8000)
            return false
        end
    end
    return true
...
function func.KeyCard()
	...
	if user_id then
        if vRP.getInventoryItemAmount(user_id,"pendrive") >= 1 then
			...
			TriggerClientEvent("Notify",source,"aviso","Aviso","Você não possui o item <b>Pendrive</b>.") 
			return false
		end
	end
end
...
Citizen.CreateThread(function()
    while true do
        ...
        if os.date("%H:%M:%S") == "20:30:00" or os.date("%H:%M:%S") == "02:00:00" or os.date("%H:%M:%S") == "16:30:00" then
            ...
            TriggerEvent("DropSystem:create","maconha",1,spawns[selecionado].x,spawns[selecionado].y,spawns[selecionado].z,3600)
        end
    end
end)