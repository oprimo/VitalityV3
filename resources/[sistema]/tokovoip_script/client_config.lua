TokoVoipConfig = {
	...
	plugin_data = {
		...
		TSChannel = "[JOGANDO]",
		TSPassword = "159753", -- TeamSpeak channel password (can be empty)
		...
		TSServer = "54.39.211.23:9004", -- TeamSpeak server address to be displayed on blocking screen
...
AddEventHandler("onClientResourceStart", function(resource)
	if (resource == GetCurrentResourceName()) then	--	Initialize the script when this resource is started
		Citizen.CreateThread(function()
			TokoVoipConfig.plugin_data.localName = " Vitality Returned"
		end);
