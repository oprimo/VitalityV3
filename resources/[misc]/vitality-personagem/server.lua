...
function processSpawnController(source,statusSent,user_id)
	...
	elseif statusSent == 1 or statusSent == 0 then
		userlogin[user_id] = true
		TriggerClientEvent("vitality-personagem:characterCreate",source)
	end
end

RegisterServerEvent("vitality-personagem:finishedCharacter")
AddEventHandler("vitality-personagem:finishedCharacter",function(characterNome,characterSobrenome,characterAge,currentCharacterMode)
	local source = source
...
function doSpawnPlayer(source,user_id,firstspawn)
	TriggerClientEvent("vitality-personagem:normalSpawn",source,firstspawn)
