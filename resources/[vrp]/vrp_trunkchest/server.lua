...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookbaucarro = "https://discord.com/api/webhooks/784773426576359446/-zpQXbUFUtmVfcBVe3xT1zL6eM_jRVKj-zVGTvz1ORqYTTzDGX9ZQ_S4HdeWs3hRq2Gj"
	...
	if itemName then
		...
		if user_id then
			if string.match(itemName,"dinheirosujo") or string.match(itemName,"identidade") then
				TriggerClientEvent("Notify",source,"importante","Importante","Não pode guardar este item em veículos.",8000)
				return
			end
			...
			if items then
				...
				if parseInt(amount) > 0 then
					...
					if new_weight <= parseInt(max_veh) then
						...
						TriggerClientEvent("Notify",source,"negado","Negado","<b>Porta-Malas</b> cheio.",8000)
					end
				else
