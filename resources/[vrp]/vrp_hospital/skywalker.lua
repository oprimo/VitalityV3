...
RegisterCommand('resgate', function(source,args,rawCommand)
	...
	if parseInt(#colaboradordmla) == 0 then
		TriggerClientEvent("Notify",source,"importante","Importante", "Não há <b>colaboradores do departamento médico</b> em serviço no momento.")
	elseif parseInt(#colaboradordmla) == 1 then
		TriggerClientEvent("Notify",source,"importante","Importante", "Atualmente, <b>"..paramedicos.." colaborador do departamento médico</b> está em serviço.")
	elseif  parseInt(#colaboradordmla) >= 1 then
		TriggerClientEvent("Notify",source,"importante","Importante", "Atualmente, <b>"..paramedicos.." colaboradores do departamento médico</b> estão em serviço.")
	end
...
AddEventHandler("reanimar:pagamento",function()
	...
	if user_id then
		...
		TriggerClientEvent("Notify",source,"sucesso","Sucesso","Recebeu <b>$"..pagamento.." dólares</b> de gorjeta do americano.")
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- [ RE ] -------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand('re',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"dmla.permissao") then
		...
		if nplayer then
			...
			TriggerClientEvent("Notify",source,"importante","Importante","Chegue mais perto do paciente.")
		end
    ...
    if vRP.hasPermission(user_id,"dmla.permissao") then
        ...
        if nplayer then
			if not vRPclient.isComa(nplayer) then
				...
				TriggerClientEvent("Notify",source,"sucesso","Sucesso","Tentando tratar o paciente.",10000)
            end
        end
    end
end)
