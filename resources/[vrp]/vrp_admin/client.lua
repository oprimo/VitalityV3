...
AddEventHandler("vehtuning",function()
	...
	if IsEntityAVehicle(vehicle) then
        ...
        SetVehicleNumberPlateText(vehicle,"PRIMO")
...
AddEventHandler("SpecMode", function(nsource)
    if not NetworkIsInSpectatorMode() and nsource then
        ...
        TriggerEvent("Notify", "sucesso", "Sucesso", "Você entrou no modo espectador.")
    else
        TriggerEvent("rsh:ExcecaoSpec", false)
        NetworkSetInSpectatorMode(false)
        TriggerEvent("Notify", "negado", "Negado", "Você saiu do modo espectador.") 
    end
end)