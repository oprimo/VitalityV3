		...
		if huser_id then
			if huser_id == user_id then
				if not vRP.accessHome(user_id,home_name,number) then
					TriggerClientEvent("Notify",player,"importante","Importante","Residência ocupada no momento.")
				end
			else
				...
				if hplayer ~= nil then
					...
					if vRP.request(hplayer,"<b>"..identitys.name.." "..identitys.firstname.."</b> está tocando seu interfone, deseja aceitar?",30) then
						...
						TriggerClientEvent("Notify",player,"negado","Negado","Recusaram sua entrada.")
					end
				end
			end
		else
			TriggerClientEvent("Notify",player,"aviso","Aviso","Interfone inválido.")
		end
	end }
	...
	menu["Vender"] = { function(player,choice)
		if vRP.request(player,"Você deseja <b>vender</b> sua residência?",30) then
			...
			if owner then
				...
				TriggerClientEvent("Notify",player,"sucesso","Sucesso","Venda concluida.")
			else
				TriggerClientEvent("Notify",player,"negado","Negado","Residência não encontrada.")
			end
		end
