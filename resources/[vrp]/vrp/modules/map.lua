...
function vRP.searchReturn(source,user_id)
	if search[user_id] == 0 or not search[user_id] then
		...
		TriggerClientEvent("Notify",source,"importante","Importante","Aguarde "..vRPclient.getTimeFunction(source,parseInt(search[user_id]))..".",8000)
		return true
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand('procurado',function(source,args,rawCommand)
	...
	if user_id then
		if search[user_id] == 0 or not search[user_id] then
			TriggerClientEvent("Notify",source,"importante","Importante","Você não está sendo procurado.",10000)
		else
			TriggerClientEvent("Notify",source,"importante","Importante","Aguarde "..vRPclient.getTimeFunction(source,parseInt(search[user_id]))..".",8000)
		end
	end
