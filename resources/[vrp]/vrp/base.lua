...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local logEntrada = "https://discord.com/api/webhooks/784772070448955392/WrahkDuMayBoxXxGLOm97AZFpUPpgaU7WIiVr5-sMxSliwfsccG3HbkTwMMTCdWTa3KI"
local logSaida = "https://discord.com/api/webhooks/784772280617402368/NjFLjZgT5ji3XoS7S8DUmQiyL-r9mOsA5UOuGcRA6Uxnw2VCnj6iRQRi_ijzpCoN0D5K"
local policiaPonto = ""
local resgatePonto = ""
-----------------------------------------------------------------------------------------------------------------------------------------
-- BASE.LUA
			...
			PerformHttpRequest(logSaida, function(err, text, headers) end, 'POST', json.encode({
				embeds = {
					{ 
						...
						thumbnail = {
							url = "https://i.imgur.com/Pyb1JxV.png"
						}, 
						fields = {
							{ 
								name = "**ID:**", 
						...
						footer = { 
							...
							icon_url = "https://i.imgur.com/Pyb1JxV.png" 
						},
						color = 15914080 
					}
				}
...
AddEventHandler("queue:playerConnecting",function(source,ids,name,setKickReason,deferrals)
	...
	if ids ~= nil and #ids > 0 then
		deferrals.update("Carregando os pevepas.")
		local user_id = vRP.getUserIdByIdentifiers(ids)
		if user_id then
			deferrals.update("Carregando os cria.")
			if not vRP.isBanned(user_id) then
				deferrals.update("Banindo os xitado.")
				if vRP.isWhitelisted(user_id) then
					if vRP.rusers[user_id] == nil then
						deferrals.update("Carregando as info braba.")
					...
					deferrals.done("Cê tá sem WL cria, lança lá no disc r$! [ discord.gg/vreturned ][ ID: "..user_id.." ]")
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
				deferrals.done("Você foi de VAPO CRIA! [ Mais informações em: discord.gg/vreturned ] ")
				--print(user_id)
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
