...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookbaucasas = "https://discord.com/api/webhooks/784773532362080257/RSbzW8n-X09fHhqxraq05IQlHvhOCyxteXYytFFTLjlr2Cfkg2VPEvnACEoWCY4kSnTW"
			...
			if ownerHomes[1] then
				--if not vRP.hasGroup(user_id,"Platina") then
					...
					if vRP.tryFullPayment(user_id,parseInt(house_price * house_tax)) then
						...
						TriggerClientEvent("Notify",source,"sucesso", "Sucesso","Pagamento de <b>$"..vRP.format(parseInt(house_price * house_tax)).." dólares</b> efetuado com sucesso.",10000)
					else
						TriggerClientEvent("Notify",source,"negado", "Negado","Dinheiro insuficiente.",10000)
					end
				--end
				...
				for k,v in pairs(myHomes) do
					...
					if ownerHomes[1] then
						...
						if parseInt(os.time()) >= parseInt(ownerHomes[1].tax+24*15*60*60) then
							TriggerClientEvent("Notify",source,"negado", "Negado","<b>Residência:</b> "..v.home.."<br><b>Property Tax:</b> Atrasado<br>Valor: <b>$"..vRP.format(parseInt(house_price * house_tax)).." dólares</b>",20000)
						else
							TriggerClientEvent("Notify",source,"importante", "Importante","<b>Residência:</b> "..v.home.."<br>Taxa em: "..vRP.getDayHours(parseInt(86400*15-(os.time()-ownerHomes[1].tax))).."<br>Valor: <b>$"..vRP.format(parseInt(house_price * house_tax)).." dólares</b>",20000)
						end
						Citizen.Wait(10)
					end
				end
