...
Citizen.CreateThread(function()
	while true do
		...
		local hsByMK2 = HasPedBeenDamagedByWeapon(PlayerPedId(), -1075685676, 0)
		local hsByCBPS = HasPedBeenDamagedByWeapon(PlayerPedId(), 1593441988, 0)
		local a, b = GetPedLastDamageBone(PlayerPedId())
		if a and b == 31086 then
			if hsByAk or hsByG3 or hsByMK2 or hsByCBPS then
				SetEntityHealth(PlayerPedId(), 101)
			elseif hsByMpx or hsByM4 then
				SetEntityHealth(PlayerPedId(), GetEntityHealth(PlayerPedId()) - 101)
			end
			a, b, hsByAk, hsByMpx, hsByG3, hsByM4, hsByMK2, hsByCBPS = nil, nil, nil, nil, nil, nil, nil, nil
		end
	end
end)
