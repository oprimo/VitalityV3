	...
	if user_id then
		...
		local tattoshop_choice = function(player,choice)
			...
			if tattoo then
				...
				if tattoo == "CLEAR" then
					...
					if ok then
						TriggerClientEvent("Notify",source,"sucesso","Sucesso","Limpeza concluida.")
					...
					if not applied then
						TriggerClientEvent("Notify",source,"sucesso","Sucesso","Tatuagem adicionada.")
						TSclient.addTattoo(source,tattoo,shop)
						TriggerClientEvent("reloadtattos",source)
					else
						TriggerClientEvent("Notify",source,"negado","Negado","Tatuagem removida.")
		...
		menudata.onclose = function(player)
			...
			if vRP.tryPayment(user_id,price) then
				...
				if price > 0 then
					TriggerClientEvent("Notify",source,"sucesso","Sucesso","Pagou <b>$"..price.." dólares</b>.")
				end
			else
