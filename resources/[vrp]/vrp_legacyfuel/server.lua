	...
	if user_id and price > 0 then
		if vRP.tryPayment(user_id,price) then
			if galao then
				...
				TriggerClientEvent("Notify",source,"sucesso", "Sucesso","Pagou <b>$"..vRP.format(price).." dólares</b> pelo <b>Galão</b>.",8000)
			else
				TriggerClientEvent('syncfuel',-1,vehicle,fuel)
				TriggerClientEvent("Notify",source,"sucesso","Sucesso","Pagou <b>$"..vRP.format(price).." dólares</b> em combustível.",8000)
			end
		else
