...
function dragAndDrop() {
  ...
  $(".trunk-workspace .objects").droppable({
    ...
    drop: function(event, ui) {
      $(this).animate({'opacity': 1}, 200);
      var $el = $(ui.draggable);
      var el_key = $el.data("item-key");
      var el_amount = $el.data("item-amount");
      $.post("http://vrp_chest/storeItem",
        JSON.stringify({
          item: el_key,
          el_amount
        })
...
var templateTrunkChest = (key, amount, image, name, weight, target) => {
  ...
  return `
  <div class="cell" data-item-key="${key}" data-item-amount="${amount}">
    ...
    <div class="row">
      <span class="amount"><i>(${(weight * amount).toFixed(2)}kg)</i></span>
    </div>
    <div class="row">
      <div class="image" style="background-image: url(nui://vitality_inventory/nui/images/${image}.png)"></div>
    </div>
    <div class="row">
      <div class="name">${name} ${formatarNumero(amount)}x</div>
    </div>
  </div>
  `;
}
...
function resizeIfNeeded() {
  dragAndDrop()
  var cellsInventory = $(".bag-workspace .row.objects .cell").length;
  var cellsChest = $(".trunk-workspace .row.objects .cell").length;
  if (cellsInventory == 0) {
    $(".bag-workspace .row.objects").css("min-height", "400px");
  } else {
    $(".bag-workspace .row.objects").css("min-height", "auto");
  }

  if (cellsChest == 0) {
    $(".trunk-workspace .row.objects").css("min-height", "400px");
  } else {
    $(".trunk-workspace .row.objects").css("min-height", "auto");
  }
}
...
$(document).ready(function() {
  $(document).on('mousedown', '.objects .cell', function(ev){
    if(ev.which == 3) {
      $('.amount-option').hide();
      $(this).find('.amount-option').show("clip", {}, 150);
    }
  });
