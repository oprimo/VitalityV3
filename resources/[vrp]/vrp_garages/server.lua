...
--[ WEBHOOK ]----------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookadmin = "https://discord.com/api/webhooks/784790390271508481/ItY43Ju2hz_ZEQDgtFw_OFNGRhO-UY3uJu8KfGqcHwPoYUMYPXgCCZlZo3TAaZwK5mzb"
local webhookvehs = "https://discord.com/api/webhooks/784790390271508481/ItY43Ju2hz_ZEQDgtFw_OFNGRhO-UY3uJu8KfGqcHwPoYUMYPXgCCZlZo3TAaZwK5mzb"
local webhookdesmanche = "https://discord.com/api/webhooks/784790390271508481/ItY43Ju2hz_ZEQDgtFw_OFNGRhO-UY3uJu8KfGqcHwPoYUMYPXgCCZlZo3TAaZwK5mzb"
...
local garages = {
	...
	[37] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
	[38] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
	[39] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
	[32] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
	[36] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
	[35] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
	[34] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
	[33] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
	[32] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
	[31] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
...
-----------------------------------------------------------------------------------------------------------------------------------------
	...
	[21] = { ['name'] = "CARTEL", ['payment'] = false, ['public'] = true },
-----------------------------------------------------------------------------------------------------------------------------------------
---[ FORTHILLS ]-------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
...
--	[614] = { ['name'] = "DMLAH", ['payment'] = false, ['perm'] = "dmla.permissao" },
--	[615] = { ['name'] = "News", ['payment'] = false, ['perm'] = "news.permissao" },
--	[616] = { ['name'] = "NewsH", ['payment'] = false, ['perm'] = "news.permissao" },
	[617] = { ['name'] = "Bikes", ['payment'] = false, ['public'] = true },
	[618] = { ['name'] = "Pesca", ['payment'] = false, ['public'] = true },
}
-----------------------------------------------------------------------------------------------------------------------------------------
--[ GARAGEMS ]---------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
			...
			if #address > 0 then
				...
				for k,v in pairs(vehicle) do
					...
					elseif vehicle[k].detido == 1 then
						...
						status = "<span class=\"green\">$"..vRP.format(parseInt(vRP.vehiclePrice(vehicle[k].vehicle)*0.1)).."</span>"
					end
		...
		if user_id then
			...
			if not vCLIENT.returnVehicle(source,name) then
				...
				if vehicle[1] ~= nil then
					if parseInt(os.time()) <= parseInt(vehicle[1].time+24) then
							...
							if vRP.vehicleType(tostring(name)) == "exclusive" or vRP.vehicleType(tostring(name)) == "rental" then
								...
								if ok then
									if vRP.tryFullPayment(user_id,price_tax) then
										vRP.execute("creative/set_ipva",{ user_id = parseInt(user_id), vehicle = name, ipva = parseInt(os.time()) })
										TriggerClientEvent("Notify",source,"financeiro","Financeiro","Pagamento do <b>Vehicle Tax</b> conclúido com sucesso.",10000)
									else
										TriggerClientEvent("Notify",source,"negado","Negado","Dinheiro insuficiente.",10000)
									end
								end
...
RegisterCommand('dv',function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"mindmaster.permissao") or vRP.hasPermission(user_id,"administrador.permissao") or vRP.hasPermission(user_id,"suporte.permissao") then
	...
	if user_id then
		...
		if vehicle and placa then
			...
			if user_id == placa_user_id then
				...
				if lock == 1 then
					TriggerClientEvent("Notify",source,"importante","Importante","Veículo <b>trancado</b> com sucesso.",8000)
				else
					TriggerClientEvent("Notify",source,"importante","Importante","Veículo <b>destrancado</b> com sucesso.",8000)
				end
			end
		end
	end
		...
		if args[1] and parseInt(args[2]) > 0 then
			...
			if myvehicles[1] then
				if vRP.vehicleType(tostring(args[1])) == "exclusive" or vRP.vehicleType(tostring(args[1])) == "rental" and not vRP.hasPermission(user_id,"administrador.permissao") then
					TriggerClientEvent("Notify",source,"negado","Negado","<b>"..vRP.vehicleName(tostring(args[1])).."</b> não pode ser transferido por ser um veículo <b>Exclusivo ou Alugado</b>.",10000)
				else
						...
						if vRP.request(nplayer,"Aceita comprar um <b>"..vRP.vehicleName(tostring(args[1])).."</b> de <b>"..identity2.name.." "..identity2.firstname.."</b> por <b>$"..vRP.format(parseInt(price)).."</b> dólares ?",30) then
							...
							if parseInt(price) > 0 then
								if vRP.tryFullPayment(parseInt(args[2]),parseInt(price)) then
									if vehicle[1] then
										TriggerClientEvent("Notify",source,"negado","Negado","<b>"..identity.name.." "..identity.firstname.."</b> já possui este modelo de veículo.",10000)
									else
										...
										TriggerClientEvent("Notify",source,"sucesso","Sucesso","Você Vendeu <b>"..vRP.vehicleName(tostring(args[1])).."</b> e Recebeu <b>$"..vRP.format(parseInt(price)).."</b> dólares.",20000)
										TriggerClientEvent("Notify",nplayer,"importante","Importante","Você recebeu as chaves do veículo <b>"..vRP.vehicleName(tostring(args[1])).."</b> de <b>"..identity2.name.." "..identity2.firstname.."</b> e pagou <b>$"..vRP.format(parseInt(price)).."</b> dólares.",40000)
										...
										TriggerClientEvent("Notify",nplayer,"negado","Negado","Dinheiro insuficiente.",8000)
				...
				if #vehicle > 0 then 
	                ...
	                TriggerClientEvent("Notify",source,"importante","Importante","Seus veículos: " .. car_names, 20000)
				else 
					TriggerClientEvent("Notify",source,"importante","Importante","Você não possui nenhum veículo.",20000)
				end
			end
		end
	end)
